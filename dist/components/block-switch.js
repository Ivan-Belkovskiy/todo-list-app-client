        ...
        if (forElement) {
            ...
            selectElements.forEach((Element) => {
                if (forElement instanceof Element) {
                    const thisChildNodes = Array.from(this.shadowRoot.querySelectorAll('block-switch-option'));
                    ...
                    forChildNodes.forEach((node) => {
                        ...
                        if (node instanceof HTMLOptionElement) {
                            ...
                            // }
                            const element = this.shadowRoot.querySelector(`block-switch-option[value="${node.value}"]:not(other-value)`);
                            const otherValue = this.shadowRoot.querySelector(`block-switch-option[other-value]`);
