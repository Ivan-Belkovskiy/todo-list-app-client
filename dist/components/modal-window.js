...
class ModalWindow extends HTMLElement {
    ...
    initElement() {
        if (!this.shadowRoot)
            return;
        const colorType = this.getAttribute('color') || 'default';
        const colorTypes = {
            'default': '#e6e6e6',
            'secondary': '#000000',
        };
        ...
        if (this.modalElement && !this.hasAttribute('opened'))
        ...
        const openBtn = (this.getAttribute('open-button') != null) ? document.querySelector(this.getAttribute('open-button') || '') : null;
        if (openBtn) {
    ...
    close() {
        ...
        const animationEndListener = (e) => {
            if (!content.style.animationDirection.includes('reverse'))
                return;
            ...
            if (e.elapsedTime >= 0.75) {
                ...
                ;
                content.removeEventListener('animationend', (event) => animationEndListener(event));
            }
        };
        if (typeof this.getAttribute('hide-animation') === 'string' && this.getAttribute('hide-animation') != 'none') {
            content.addEventListener('animationend', (event) => animationEndListener(event));
        }
        else {
            if (!this.modalElement.style.animation)
                this.modalElement.style.display = 'none';
            if (this.closeButton)
                this.closeButton.disabled = false;
        }
